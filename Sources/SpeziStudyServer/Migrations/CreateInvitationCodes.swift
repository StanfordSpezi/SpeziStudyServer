...
struct CreateInvitationCodes: AsyncMigration {
    func prepare(on database: any Database) async throws {
        try await database.schema("invitation_codes")
            ...
            .field("enrollment_id", .uuid, .references("enrollments", "id", onDelete: .cascade))
            // Keycloak user ID of the admin who issued the code (not a DB foreign key since users live in Keycloak)
            .field("issued_by", .string, .required)
